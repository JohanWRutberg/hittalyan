...
export function ChanceMeter({
  ...
  const title = hasRange
    ? t("meterTitle", {
        ...
        q1: q1!,
        q3: q3!,
        user: userYears != null ? t("meterUser", { years: formatYearsShort(userYears, locale) }) : "",
      })
    : t(`${c.level}.hint`);
  ...
  return (
    <div className="flex items-center gap-2.5" title={title}>
      ...
      {!compact && hasRange && <span className="shrink-0 text-xs text-muted">{t("range", { q1: q1!, q3: q3! })}</span>}
    </div>
  );
}
